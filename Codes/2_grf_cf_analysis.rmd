...
agg_reg_data <-
  copy(reg_data) %>%
  .[year %in% 2008:2012 & wellid %in% comp_wellid] %>%
  # .[year %in% 2008:2012] %>%
...
agg_reg_data2[,treat2] %>% unique()

```


```{r}
# /*===== Check how often farmers in LR use more than 9 inches =====*/
ch_reg_data <- 
  here("Shared/Data/WaterAnalysis/comp_reg_dt.rds") %>%
  readRDS() %>%
...
cov_ls <- c(
  ...
  "pr_in",  "pet_in",
  # --- soil --- #
  "claytotal_r", "slope_r", "awc_r"
  )


# data <- reg_data
data <- agg_reg_data
data <- strict_reg_data
data <- strict_agg_reg_data
