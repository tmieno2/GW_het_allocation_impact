...
# CF Analysis using only 1st allocation period: Annual Data
...
T <- data[, treat2]
X <- data[, ..cov_ls]
X_pred <- data[, c(cov_ls,"gdd_in", "sandtotal_r"), with = FALSE]
cl_var <- data[,tr_year]
...
+ (1) A farmer was allowed to allocate his total allocation limit (45 inches-acre) freely across years within an allocation period, A Smart farmers would have saved some of his annual groundwater usage to prepare future weather uncertainty. 
...
+ **Question**
+ Is there any pattern in farmers who used much more amount of groundwater than 9 inches-acre ?
+ 
...
strict_reg_data <-
  rbind(strict_trt_grp, con_grp)


strict_comp_wellid <- 
  strict_reg_data %>%
  .[year %in% 2008:2012, .(wellid, year)] %>%
  dcast(wellid ~ year, value.var = "year") %>%
  na.omit() %>%
  .[,wellid]


strict_agg_reg_data <-
  copy(strict_reg_data) %>%
  .[wellid %in% strict_comp_wellid] %>%
  # .[year %in% 2008:2012] %>%
...
# in the previous code, some rows has T=0.8

strict_agg_reg_data[,treat2] %>% unique()
```
