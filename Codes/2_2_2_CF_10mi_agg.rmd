---
title: "CF analysis, 10 miles, Aggregated Data"
...
here::i_am("GitControlled/Codes/2_2_2_CF_10mi_agg.rmd")
...
## Aggragate annual data to whole-periods data by taking mean
...
TB_10mi_well <-
  ...
  .[, names(LR_10mi_owner), with=FALSE]
```

### Regression data

```{r}
reg_data_10mi <- bind_rows(LR_10mi_owner, TB_10mi_well)
```
...
# === LR === #
check_comp_LR <- 
  LR_10mi_y_owner[,.(nrd_owner_name, year)] %>%
  .[,index := 1] %>%
...
# select nrd_owner_name who has complete 5 years data
...
# === LR === #
LR_10mi_owner_comp <- 
  LR_10mi_y_owner[nrd_owner_name %in% all_y_LR_owner,] %>%
  .[,.(
  usage =  sum(usage),
  treat2 = mean(treat2),
  # --- soil --- #
...
# === TB === #
TB_10mi_well_comp <-
  TB_10mi[wellid %in% all_y_TB_owner,] %>%
  .[,.(
  usage = sum(usage),
  treat2 = mean(treat2),
  # --- soil --- #
...
```{r}
reg_data_10mi_comp <- bind_rows(LR_10mi_owner_comp, TB_10mi_well_comp)
```
