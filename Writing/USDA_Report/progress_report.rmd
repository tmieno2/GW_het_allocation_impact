...
author: Shunkei Kakimoto^[Department of Agricultural Economics, University of Nebraska-Lincoln 102 Filley Hall 1625 Arbor Drive Lincoln, NE 68583, USA, E-mail address; skakimoto3@hunskers.unl.edu] and Taro Mieno^[Department of Agricultural Economics, University of Nebraska-Lincoln, Lincoln, NE 68583, USA]
output:
...
# Summary
+ Analysis focused on estimating the heterogeneous impact of groundwater allocation policy on farmers' irrigation activity in Nebraska during 2008-2015, and revealing which factor is the main driver to cause such a heterogeneity.
...
+ This implies that LR may be able to enhance farming profitability by devising spatially heterogeneous allocation limits within the NRD with increased (decreased) allocation limits based on spatial gradients in precipitation, while maintaining the same level of overall groundwater use as the current uniform allocation limit design.
...
+ We control weather and soil conditions in the regression analysis. For weather variables, we obtained total precipitation (in mm) and total reference grass evaportranspiration (in mm) for the growing season (April to September) from daily weather records of gridMET database. For soil characteristics data, we collected the percentage of silt, clay, as well as hydraulic conductivity (in um/s), water holding capacity (in cm/cm), and slope (in percentage) from the the Soil Survey Geographic (SSURGO) database. 
