...
## Spatial Distribution of awc

```{r}
LR_5mi_awc <-
  copy(LR_5mi) %>%
  unique(., by = c("latdd", "longdd")) %>%
  .[, awc_index := ifelse(awc_r > 0.21, "High", "Low")] %>%
  st_as_sf(., coords = c("longdd","latdd"), crs = 4269)

vis_awc <- 
  ggplot() +
    geom_sf(data=nrd_bound %>% filter(NRD_Name == "Lower Republican")) +
    geom_sf(data=LR_5mi_awc, aes(color=awc_index), size=0.5) +
    ...
    guides(color = guide_legend(title = "Level of awc", override.aes = list(size=1))) +
    scale_color_hue(
      labels = c(
        "Higher than 0.21",
        "Lower than 0.21")
